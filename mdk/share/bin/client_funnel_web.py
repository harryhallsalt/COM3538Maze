...
class controller:
	...
	def subscribe(self, enable):
		...
		if enable:
			...
			#topic = self.topic_base_name + "/vision/annotation"
			#print ("subscribe", topic)
			#self.sub_img_ann = rospy.Subscriber(topic, miro.msg.img_annotation, self.callback_img_ann, queue_size=1, tcp_nodelay=True)
	...
	def loop(self):
		...
		while self.active and not rospy.core.is_shutdown():
			...
			if not self.package is None:
				...
				#self.msg_funnel_web.img_ann = self.img_ann
				if self.tone is None:
